...
void Scene3::Update(ID3D11DeviceContext* deviceContext, InputClass& kbd, Mouse& mouse, float dt, float tt) {
	...
	//m_Cam->SetPosition(0.0f, 96.0f, 0.0f);
	//m_sceneTree->SetSceneNumber(1);
	//m_Cam->SetPosition(0.0f, 2.0f, 0.0f);
	...
	unsigned int actId5 = 17;
	std::shared_ptr<Actor> act5 = MakeStrongPtr(SystemClass::GetEngineLogic()->VGetActor(actId5));
	if (act5) {
		unsigned int componentId = ActorComponent::GetIdFromName("TransformComponent");
		std::shared_ptr<TransformComponent> rc = MakeStrongPtr(act5->GetComponent<TransformComponent>(componentId));
		float factorX = rc->GetPosition3f().x;
		float factorY = rc->GetPosition3f().y;
		float factorZ = rc->GetPosition3f().z;
		float factorYaw = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().x) + dt * 16.0f;
		...
		std::shared_ptr<EvtData_Move_Actor> pMoveActorEvent(new EvtData_Move_Actor(actId5, m_testMatrix));
		IEventManager::Get()->VQueueEvent(pMoveActorEvent);
	}
	...
	unsigned int actId7 = 20;
	std::shared_ptr<Actor> act7 = MakeStrongPtr(SystemClass::GetEngineLogic()->VGetActor(actId7));
	if (act7) {
		if (tt >= 22 && tt < 29.0f) {
			unsigned int componentId = ActorComponent::GetIdFromName("TransformComponent");
			std::shared_ptr<TransformComponent> rc = MakeStrongPtr(act7->GetComponent<TransformComponent>(componentId));
			float factorX = 86.9f;
			float factorY = 2.0f;
			float factorZ = pos + 3.0f;
			float factorYaw = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().x);
			float factorPitch = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().y);
			float factorRoll = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().z) + dt * 12.0f;
			DirectX::XMFLOAT4X4 m_testMatrix;
			DirectX::XMStoreFloat4x4(
				&m_testMatrix,
				DirectX::XMMatrixMultiply(
			...
			std::shared_ptr<EvtData_Move_Actor> pMoveActorEvent(new EvtData_Move_Actor(actId7, m_testMatrix));
			IEventManager::Get()->VQueueEvent(pMoveActorEvent);
		}
		if (tt >= 29.0f) {
			unsigned int componentId = ActorComponent::GetIdFromName("TransformComponent");
			...
			float factorX = 0.5f;
			float factorY = 2.0f;
			float factorZ = pos + 1.5f;
			float factorYaw = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().x);
			float factorPitch = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().y);
			float factorRoll = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().z) + dt * 16.0f;
			DirectX::XMFLOAT4X4 m_testMatrix;
			DirectX::XMStoreFloat4x4(
				&m_testMatrix,
				DirectX::XMMatrixMultiply(
	...
	unsigned int actId6 = 19;
	std::shared_ptr<Actor> act6 = MakeStrongPtr(SystemClass::GetEngineLogic()->VGetActor(actId6));
	if (act6) {
		if (tt >= 15.0f && tt < 21.5f) {
			unsigned int componentId = ActorComponent::GetIdFromName("TransformComponent");
			std::shared_ptr<TransformComponent> rc = MakeStrongPtr(act6->GetComponent<TransformComponent>(componentId));
			float factorX = 86.9f;
			float factorY = 2.0f;
			float factorZ = pos + 3.0f;
			float factorYaw = DirectX::XMConvertToDegrees(rc->GetYawPitchRoll3f().x);
			...
			std::shared_ptr<EvtData_Move_Actor> pMoveActorEvent(new EvtData_Move_Actor(actId6, m_testMatrix));
			IEventManager::Get()->VQueueEvent(pMoveActorEvent);
		}
		if (tt >= 21.5f) {
			unsigned int componentId = ActorComponent::GetIdFromName("TransformComponent");
