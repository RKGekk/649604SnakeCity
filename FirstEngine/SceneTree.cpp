...
SceneTree::SceneTree(const EngineOptions& options, ID3D11Device* device, ID3D11DeviceContext* deviceContext) {
	...
	m_Offscreen.reset(new D3D11Offscreen(nullptr, device, L"PostPS.fx"));
	m_Sky.reset(new D3D11Sky(nullptr, device, L"10.dds"));
	m_Sky2.reset(new D3D11Sky(nullptr, device, L"11.dds"));
...
HRESULT SceneTree::OnRender(ID3D11DeviceContext* deviceContext) {
	...
	if (m_Root && m_Camera) {
		...
		if (m_sceneNumber == 0) {
			if (m_Sky->VPreRender(this, deviceContext) == S_OK) {
				m_Sky->VRender(this, deviceContext);
				m_Sky->VRenderChildren(this, deviceContext);
				m_Sky->VPostRender(this);
			}
		}

		if (m_sceneNumber == 1) {
			if (m_Sky2->VPreRender(this, deviceContext) == S_OK) {
				m_Sky2->VRender(this, deviceContext);
				m_Sky2->VRenderChildren(this, deviceContext);
				m_Sky2->VPostRender(this);
			}
		}
...
void SceneTree::SetSceneNumber(int sc) {
	m_sceneNumber = sc;
}

int SceneTree::GetSceneNumber() {
