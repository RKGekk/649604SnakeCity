...
D3D11Offscreen::D3D11Offscreen(DirectX::XMFLOAT4X4* pMatrix, ID3D11Device* device, const std::wstring& ps_file) : D3D11Drawable(nullptr, pMatrix) {
	...
	for (UINT i = 0; i < quad.Vertices.size(); ++i) {
		vertices.vertices.push_back(
			Vertex(
				...
				DirectX::XMFLOAT4(SystemClass::GetGameTimer().DeltaTime(), SystemClass::GetGameTimer().TotalTime(), SystemClass::GetGameTimer().DeltaTime(), SystemClass::GetGameTimer().TotalTime())
			)
		);
	}
	...
	ShaderClass& ps = ShaderHolder::GetShader(ps_file);
